...
class LoaderSystem(ClientSystem):
    """ QuMod加载器系统
    ...
    def getSystem():
        ...
        system = clientApi.GetSystem(LoaderSystem.namespace, LoaderSystem.systemName)
        if system:
            return system
        return clientApi.RegisterSystem(LoaderSystem.namespace, LoaderSystem.systemName, LoaderSystem.__module__ + "." + LoaderSystem.__name__)
    ...
    def regSystemInit(self):
        ...
        if self._regInitState:
            return
        from ...Client import CLI_SYSTEM_APPEND
