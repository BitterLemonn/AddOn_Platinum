...
class ItemFactory(object):
    class __UserDataFactory(object):
        ...
        def addEnchantment(self, enchData):
            ...
            for ench in self.__ench:
                if isinstance(enchData.id, int):
                    if ench.get("id", {}).get("__value__") == enchData.id:
                        ench["lvl"]["__value__"] = enchData.level
                        return self
                else:
                    if ench.get("modEnchant", {}).get("__value__") == enchData.id:
                        ench["lvl"]["__value__"] = enchData.level
                        return self
            # 如果不存在相同的附魔，则添加新的附魔
        ...
        def removeEnchantment(self, enchId):
            ...
            for i, enchData in enumerate(self.__ench):
                if isinstance(enchId, int):
                    if enchData.get("id", {}).get("__value__") == enchId:
                        self.__ench.pop(i)
                        break
                else:
                    if enchData.get("modEnchant", {}).get("__value__") == enchId:
                        self.__ench.pop(i)
                        break
            if not self.__ench:
                self.__ench = None
        ...
        def setCustomData(self, data):
            self.__customData = NBTTransformer.transform({key: pickle.dumps(value) for key, value in data.items()})
            return self
    ...
    def __init__(self, itemDict=None):
        ...
        self.__durability = self.__itemDict.get("durability", None)  # type: int | None
    ...
    @classmethod
    def compireItems(cls, itemDict1, itemDict2):
        """比较两个物品字典是否相等"""
        if itemDict1["newItemName"] != itemDict2["newItemName"]:
            return False
        if itemDict1["newAuxValue"] != itemDict2["newAuxValue"]:
            return False
        if itemDict1["count"] != itemDict2["count"]:
            return False
        durability1 = itemDict1.get("durability", None)
        durability2 = itemDict2.get("durability", None)
        if durability1 != durability2:
            return False
        userData1 = itemDict1.get("userData", None)
        userData2 = itemDict2.get("userData", None)
        if not commonUtils.compireDict(userData1, userData2):
            return False
        return True
    ...
    def addCustomData(self, key, value):
        ...
        :return: ItemFactory
        """
        self.__userData.addCustomData({key: pickle.dumps(zlib.compress(pickle.dumps(value)))})
        return self
